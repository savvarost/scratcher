...
async function createScratchFile(directoryViewer: DirectoryViewer, folder_name: string) {
  ...
  if (selectedType) {
    ...
    if (fileNameNew) {
      ...
      const folderPath = path.join(utils.getScratcherDirectory(), folder_name);
      utils.ensureDirectoryExists(folderPath);

      const filePath = path.join(utils.getScratcherDirectory(), folder_name, fileNameNew);
...
export function activate(context: vscode.ExtensionContext) {
  scratcherDirectory = utils.getScratcherDirectory();
  utils.ensureDirectoryExists(scratcherDirectory);

  const directoryViewer = new DirectoryViewer(context, scratcherDirectory);
  ...
  context.subscriptions.push(
    directoryViewer,
    vscode.commands.registerCommand("scratcher.createProjectScratchFile", () => createProjectScratchFile(directoryViewer)),
    vscode.commands.registerCommand("scratcher.createTotalScratchFile", () => createTotalScratchFile(directoryViewer)),
